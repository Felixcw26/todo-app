...
class ToDo:
    ...
    >>> t2 = ToDo("Literature Review", "Philosophy")
    ...
    category : str
        Logical or thematic grouping (Must be one of: ["Philosophy", "University", "Fraunhofer", "Sport", "Band", "Reading", "Room", "Financial", "Relationship","Life"]).
    ...
    def get_est_time(self) -> tuple[int]:
        """Get estimated work time in hours and minutes."""
        if self.est_time:
            hours = int(self.est_time)
            min = int((self.est_time - hours) * 60)
            return hours, min
        else:
            return 0, 0

    def get_status(self) -> str:
        """Get status of this ToDo."""
        if self.done and not self.in_progress:
            return "Done"
        elif not self.done and self.in_progress:
            return "Progress"
        elif not self.done and not self.in_progress:
            return "Undone"
        else:
            return None
    ...
    def clone(self) -> "ToDo":
        """Return a deep copy of this ToDo without IDs or dependency links."""
        new_todo = ToDo(
            title=self.title,
            category=self.category,
